...
var DB = ["bici", "bicis", "montaña", "disco", "discos", "royce", "hardwell", "libros", "harry potter", "carretera"];
...
http.createServer(function (request, response) {
  ...
  /*COMPROBAMOS QUE TENGA COOKIE, SI NO TIENE LA CREAMOS PARA MANDARLA DE NUEVO
    JUNTO CON LA RESPUESTA INICIAL*/
  var cookie = request.headers.cookie;
  if (cookie == null){
    cookie = setcookie(Math.random());
    console.log('NO TENEMOS COOKIE. HEMOS CREADO UNA' + cookie);
  }
  ...
  if (request.method == 'GET'){
      ...
      if(exists){
        ...
        fs.readFile(filePath, function(error, data) {
          if(error){
            ...
            response.writeHead(200, {'Content-Type': contentType }, {'Set-Cookie': cookie});
    ...
    if (filePath == "./buscar_BD"){
      request.on('data', function(datos){
        var producto_a_buscar = datos.toString();
        /*Creamos el JSON con el cual vamos a mandar las opciones de nuestra BD*/
        var opciones_DB = JSON.parse('{"sugerencias":[]}');
        console.log(producto_a_buscar);
        for (var i in DB){
          caso = DB[i].substring(0,producto_a_buscar.length);
          console.log("CASO: " + caso);
          if (caso == producto_a_buscar){
            console.log("ENTRA");
            opciones_DB.sugerencias.push({opcion: DB[i]});
            console.log(opciones_DB.sugerencias);
          }
        }
      response.writeHead(200, {"Content-Type": "text/plain"});
      var myJSONText = JSON.stringify(opciones_DB);
...
function setcookie(value) {
  var user = "usuario=";
  var expire = " expires=30";
  return user+value+expire;
}
        //message = value + " Product(s) with id " + productid +
